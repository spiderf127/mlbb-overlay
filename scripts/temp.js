...
setInterval(() => {
  ...
  if(raw && raw !== lastStr) {
    lastStr = raw;
    try { S = JSON.parse(raw); render(); } catch(e){}
  }
}, 300);
...
function renderTimer() {
  ...
  if (S.status === 'last_change') {
    ...
    if (lcBar) {
      lcBar.style.display = 'block';
      lcBar.style.width = (pct * 100) + '%';
    }
  } else {
    if (lcBar) lcBar.style.display = 'none';
    ...
    if(isRed) {
      bar.style.backgroundColor = 'var(--red-team)';
      bar.style.float = 'right';
    } else {
      bar.style.backgroundColor = 'var(--blue-team)';
...
function render() {
  ...
  if (bGroup && rGroup) {
    ...
    for(let i=0; i<nBars; i++) {
      ...
      bb.className = 'score-bar' + (i < bScore ? ' blue-fill' : '');
      bGroup.appendChild(bb);

      const rb = document.createElement('div');
      rb.className = 'score-bar' + (i < rScore ? ' red-fill' : '');
      rGroup.appendChild(rb);
    }
  }
  ...
  if(S.teams?.blue?.logo) {
    bLogo.innerHTML=`<img src="${S.teams.blue.logo}" class="team-logo" onerror="this.innerHTML='<div class=\\'team-logo-fallback\\'>B</div>'">`;
  } else { bLogo.innerHTML=`<div class='team-logo-fallback'>B</div>`; }
  
  if(S.teams?.red?.logo) {
    rLogo.innerHTML=`<img src="${S.teams.red.logo}" class="team-logo" onerror="this.innerHTML='<div class=\\'team-logo-fallback\\'>R</div>'">`;
  } else { rLogo.innerHTML=`<div class='team-logo-fallback'>R</div>`; }
  ...
  for(let i=0;i<bansN;i++) {
    ['blue','red'].forEach(side => {
      ...
      if(sl) {
        sl.className = 'ban-card'; // reset active class
  ...
  for(let i=0;i<picksN;i++) {
    ['blue','red'].forEach(side => {
      ...
      if(pc) pc.classList.remove('active-slot'); // Reset width and hide indicator
      
      if(hero && hero.portrait) {
        pt.innerHTML = `<img src="${hero.portrait}" class="hero-img">`;
      } else {
        if(roleImgSrc) {
          pt.innerHTML = `<img src="${roleImgSrc}" class="role-icon">`;
        } else {
          pt.innerHTML = '';
        }
      }
      ...
      if(topRoleEl) {
        if(roleImgSrc && hero) {
          topRoleEl.innerHTML = `<img src="${roleImgSrc}" class="top-role-icon">`;
          topRoleEl.style.display = 'flex';
        } else {
          topRoleEl.style.display = 'none';
        }
      }
